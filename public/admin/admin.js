  ...
  try {
    ...
    content().innerHTML = `
      ...
      <div class="grid2">
        ...
        <div>${tbl('Joiners by UTM (signups)', (d.joinersByUtm || []).map((r) => `<tr><td>${esc(r.source)} / ${esc(r.campaign)}${r.content ? ' / <b>' + esc(r.content) + '</b>' : ''}</td><td class="num">${num(r.count)}</td></tr>`).join('') || '<tr><td class="note">—</td><td></td></tr>', [{ h: 'source / campaign / ad' }, { h: 'Signups', num: 1 }])}
