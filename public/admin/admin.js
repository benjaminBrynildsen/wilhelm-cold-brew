  ...
  try {
    ...
    content().innerHTML = `
      ...
      <div class="grid2">
        ...
        <div style="grid-column:1/-1">${tbl('Conversion by ad (UTM) — landed → joined', (d.joinersByUtm || []).map((r) => `<tr><td>${esc(r.source)} / ${esc(r.campaign)}${r.content ? ' / <b>' + esc(r.content) + '</b>' : ''}</td><td class="num">${num(r.visitors)}</td><td class="num">${num(r.joined)}</td><td class="num"><b>${r.conv != null ? r.conv + '%' : '—'}</b></td></tr>`).join('') || '<tr><td class="note">—</td><td></td><td></td><td></td></tr>', [{ h: 'source / campaign / ad' }, { h: 'Landed', num: 1 }, { h: 'Joined', num: 1 }, { h: 'Conv.', num: 1 }])}
          ${d.joiners ? `<div class="note" style="margin-top:6px">${num(d.joiners.attributed)} of ${num(d.joiners.total)} joiners matched to a UTM · ${num(d.joiners.direct)} direct/untagged. "Landed" = unique visitors with that UTM; conv. = joined ÷ landed (last-touch IP-hash attribution).</div>` : ''}</div>
