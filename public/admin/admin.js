  ...
  try {
    ...
    content().innerHTML = `
      ...
      <div class="grid2">
        ...
        <div>${tbl('UTM campaigns (views)', d.topCampaigns.map((r) => `<tr><td>${esc(r.source)} / ${esc(r.medium)} / ${esc(r.campaign)}</td><td class="num">${num(r.count)}</td></tr>`).join('') || '<tr><td class="note">—</td><td></td></tr>', [{ h: 'source / medium / campaign' }, { h: 'Views', num: 1 }])}</div>
        <div>${tbl('Joiners by UTM (signups)', (d.joinersByUtm || []).map((r) => `<tr><td>${esc(r.source)} / ${esc(r.medium)} / ${esc(r.campaign)}</td><td class="num">${num(r.count)}</td></tr>`).join('') || '<tr><td class="note">—</td><td></td></tr>', [{ h: 'source / medium / campaign' }, { h: 'Signups', num: 1 }])}
          ${d.joiners ? `<div class="note" style="margin-top:6px">${num(d.joiners.attributed)} of ${num(d.joiners.total)} joiners matched to a UTM · ${num(d.joiners.direct)} direct/untagged. Attribution by IP-hash to last UTM hit before signup.</div>` : ''}</div>
